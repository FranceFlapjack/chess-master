// Move-generator correctness test: node counts for the standard perft positions.
import { Board } from '../js/engine/bot/board.js'
const cases = [
  ['startpos', 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1', [20, 400, 8902, 197281]],
  ['kiwipete', 'r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1', [48, 2039, 97862, 4085603]],
  ['pos3', '8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1', [14, 191, 2812, 43238, 674624]],
  ['pos4', 'r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1', [6, 264, 9467, 422333]],
  ['pos5', 'rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8', [44, 1486, 62379, 2103487]],
  ['pos6', 'r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10', [46, 2079, 89890, 3894594]],
]
let fail = 0, nodes = 0
const t0 = Date.now()
for (const [name, fen, expected] of cases) {
  const b = new Board(fen)
  expected.forEach((exp, i) => {
    const got = b.perft(i + 1); nodes += got
    const ok = got === exp; if (!ok) fail++
    console.log(`${ok ? '✓' : '✗'} ${name} depth ${i + 1}: ${got}${ok ? '' : ` (expected ${exp})`}`)
  })
  if (b.fen() !== fen) { fail++; console.log(`✗ ${name}: FEN round-trip mismatch: ${b.fen()}`) }
}
const s = (Date.now() - t0) / 1000
console.log(fail ? `${fail} failure(s)` : `✓ all perft counts match — ${Math.round(nodes / s / 1000)}k nodes/s`)
process.exit(fail ? 1 : 0)
